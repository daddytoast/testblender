...
def build_shell(half_tree):
    ...
    add_input(tree, "Расширение_протяжённость_доля", "NodeSocketFloat", default=0.12,
              ...
              description="Доля высоты (в T) от верхнего края, на которой расширение плавно "
                           "сходит на нет вниз.")
    add_input(tree, "Фальц_глубина_мм", "NodeSocketFloat", default=1.0,
              min_value=0.0, max_value=3.0,
              description="Паз «четверть»/фальц на швах перед/зад: глубина интерлока по "
                           "нормали (перед получает 'язычок' - локальное утолщение стенки "
                           "изнутри к наружной поверхности; зад получает ответный 'канал' - "
                           "локальное утоньшение изнутри). 0 = фальц отключён.")
    add_input(tree, "Фальц_ширина_мм", "NodeSocketFloat", default=1.0,
              min_value=0.3, max_value=8.0,
              description="Ширина плоского дна фальца вдоль стенки (по кругу), мм, у каждого "
                           "из 2 швов.")
    add_input(tree, "Фальц_утопление_мм", "NodeSocketFloat", default=0.3,
              min_value=0.0, max_value=1.0,
              description="Насколько НАРУЖНАЯ поверхность ОБЕИХ половин одинаково утоплена "
                           "у шва (симметрично - без взаимного уступа), чтобы линия стыка "
                           "была едва заметна снаружи (шов <= этой величины).")
    add_input(tree, "Фальц_зазор_мм", "NodeSocketFloat", default=0.1,
              min_value=0.0, max_value=1.0,
              description="Зазор язычок<->канал (доп. глубина канала сверх глубины язычка), "
                           "мм - место под клей и допуски печати.")
    add_input(tree, "Фальц_скос_град", "NodeSocketFloat", default=0.5,
              min_value=0.0, max_value=5.0,
              description="Угол самоцентрирующего скоса фальца вдоль длины кромки (верх-низ), "
                           "градусы - ширина плоского дна линейно меняется по высоте, чтобы "
                           "части 'входили' друг в друга при сборке.")
    add_input(tree, "Кромка_радиус_мм", "NodeSocketFloat", default=0.2,
              min_value=0.05, max_value=1.0,
              description="Ширина плавного (косинусного) перехода на границе плоского дна "
                           "фальца - приближение скругления кромки радиусом ~эта величина. "
                           "Примечание: меньше ширины линии сопла FDM-печати (~0.4мм), поэтому "
                           "на практике будет сглажено дополнительно самой печатью.")
    ...
    # ------------------------------------------------------------------
    # 3b. ФАЛЬЦ «ЧЕТВЕРТЬ» НА ШВАХ ПЕРЕД/ЗАД (точный профиль по ТЗ)
    # У каждого из 2 швов (col=0 и col=M/2) строим ПЛОСКОЕ (не купольное)
    # дно фальца шириной Фальц_ширина_мм с косинусным скруглением кромки
    # шириной Кромка_радиус_мм - т.е. plateau(col) = 1 на плоском дне,
    # гладко спускается к 0 на ширине кромки, 0 дальше. Ширина переведена
    # из мм в "колонки" через ЛОКАЛЬНУЮ калибровку мм/колонка (реальное
    # межточечное расстояние кольца), т.к. Точек_в_кольце обычно даёт
    # лишь несколько мм на колонку - при таком разрешении сетки плоское
    # дно шириной 1мм физически схлопывается в единственную ближайшую
    # колонку (что даёт резкий, но корректный по объёму шов; для
    # видимого плоского участка нужно большее Точек_в_кольце).
    #
    # НАРУЖНАЯ поверхность: у ОБЕИХ половин утапливается ОДИНАКОВО
    # (Фальц_утопление_мм) - нет взаимного уступа между половинами,
    # видимый шов <= Фальц_утопление_мм.
    #
    # ВНУТРЕННЯЯ поверхность (скрыта снаружи): Перед (male) получает
    # "язычок" - локальное утолщение стенки изнутри на Фальц_глубина_мм;
    # Зад (female) получает ответный "канал" - локальное утоньшение на
    # Фальц_глубина_мм + Фальц_зазор_мм (зазор = место под клей/допуски).
    #
    # Самоцентрирующий скос Фальц_скос_град: ширина плоского дна линейно
    # меняется вдоль высоты кромки (шире у одного конца, уже у другого),
    # что при сборке подталкивает половины к единственному правильному
    # положению.
    # ------------------------------------------------------------------
    ...
    # -- mm-калибровка: реальное расстояние (мм) между соседними точками
    # кольца рядом со швом (row=0, col=0 vs col=1) - т.к. плотность точек
    # кольца (Точек_в_кольце) даёт всего несколько мм/колонку, а фальц
    # должен быть ~1мм - переводим ширину фальца и радиус кромки из мм
    # в "колонки" через эту локальную калибровку, а не считать долей.
    n3bmmA = add_node(tree, "GeometryNodeSampleIndex", "3b.mmA",
                       "outer точка col=0,row=0 (калибровка мм/колонка)",
                       1600, -1050, data_type='FLOAT_VECTOR')
    tree.links.new(out_sock(n07b, "Geometry"), in_sock(n3bmmA, "Geometry"))
    n3bmmApos = add_node(tree, "GeometryNodeInputPosition", "3b.mmApos",
                          "Position (контекст n07b)", 1360, -1000)
    link(tree, n3bmmApos, "Position", n3bmmA, "Value_Vector")
    in_sock(n3bmmA, "Index").default_value = 0

    n3bmmB = add_node(tree, "GeometryNodeSampleIndex", "3b.mmB",
                       "outer точка col=1,row=0", 1600, -1150, data_type='FLOAT_VECTOR')
    tree.links.new(out_sock(n07b, "Geometry"), in_sock(n3bmmB, "Geometry"))
    link(tree, n3bmmApos, "Position", n3bmmB, "Value_Vector")
    in_sock(n3bmmB, "Index").default_value = 1

    n3bmmdist = add_node(tree, "ShaderNodeVectorMath", "3b.mmdist",
                          "мм/колонка (сырое) = distance(col0,col1)", 1860, -1100,
                          operation='DISTANCE')
    link(tree, n3bmmA, "Value_Vector", n3bmmdist, "Vector")
    link(tree, n3bmmB, "Value_Vector", n3bmmdist, "Vector_001")

    n3bmmclamp = add_node(tree, "ShaderNodeMath", "3b.mmclamp",
                           "мм/колонка = max(сырое, 0.5) - защита от деления на 0",
                           2120, -1100, operation='MAXIMUM')
    link(tree, n3bmmdist, "Value", n3bmmclamp, "Value")
    in_sock(n3bmmclamp, "Value_001").default_value = 0.5

    frameMM = make_frame(tree, "3b-mm. КАЛИБРОВКА ММ/КОЛОНКА (локально, у col=0)",
                          [n3bmmA, n3bmmApos, n3bmmB, n3bmmdist, n3bmmclamp])

    # -- длина кромки по Z (для скоса) и угол скоса --
    n3blenxyz = add_node(tree, "ShaderNodeSeparateXYZ", "3b.lenxyz",
                          "Z всех точек n07b (для длины кромки)", 1600, -1300)
    link(tree, n3bmmApos, "Position", n3blenxyz, "Vector")

    n3blenstat = add_node(tree, "GeometryNodeAttributeStatistic", "3b.lenstat",
                           "Мин/Макс Z по n07b", 1860, -1300, domain='POINT')
    link(tree, n07b, "Geometry", n3blenstat, "Geometry")
    link(tree, n3blenxyz, "Z", n3blenstat, "Attribute")

    n3blen = add_node(tree, "ShaderNodeMath", "3b.len", "длина_мм = Max.Z - Min.Z",
                       2120, -1300, operation='SUBTRACT')
    link(tree, n3blenstat, "Max", n3blen, "Value")
    link(tree, n3blenstat, "Min", n3blen, "Value_001")

    n3bskewrad = add_node(tree, "ShaderNodeMath", "3b.skewrad",
                           "скос в радианах", 2380, -1250, operation='RADIANS')
    tree.links.new(GI("Фальц_скос_град"), in_sock(n3bskewrad, "Value"))

    n3btan = add_node(tree, "ShaderNodeMath", "3b.tan", "tan(скос)",
                       2640, -1250, operation='TANGENT')
    link(tree, n3bskewrad, "Value", n3btan, "Value")

    n3btaperext = add_node(tree, "ShaderNodeMath", "3b.taperext",
                            "полный разброс ширины по высоте = длина_мм * tan(скос)",
                            2900, -1300, operation='MULTIPLY')
    link(tree, n3blen, "Value", n3btaperext, "Value")
    link(tree, n3btan, "Value", n3btaperext, "Value_001")

    n3bsignedT = add_node(tree, "ShaderNodeMath", "3b.signedT",
                           "signed_T = ring_T - 0.5 (-0.5..+0.5 по высоте)",
                           2380, -1400, operation='SUBTRACT')
    link(tree, n02, "Attribute_Float", n3bsignedT, "Value")
    in_sock(n3bsignedT, "Value_001").default_value = 0.5

    n3btaperoff = add_node(tree, "ShaderNodeMath", "3b.taperoff",
                            "taper_offset_мм = signed_T * разброс",
                            3160, -1350, operation='MULTIPLY')
    link(tree, n3bsignedT, "Value", n3btaperoff, "Value")
    link(tree, n3btaperext, "Value", n3btaperoff, "Value_001")

    frameTaper = make_frame(
        tree, "3b-taper. САМОЦЕНТРИРУЮЩИЙ СКОС 0.5° ВДОЛЬ ДЛИНЫ КРОМКИ (верх-низ)",
        [n3blenxyz, n3blenstat, n3blen, n3bskewrad, n3btan, n3btaperext,
         n3bsignedT, n3btaperoff])

    # -- ширина плоского дна фальца (мм, с учётом скоса) -> колонки --
    n3bwhalf0 = add_node(tree, "ShaderNodeMath", "3b.whalf0",
                          "half_width_мм = Фальц_ширина_мм * 0.5", 2380, -600, operation='MULTIPLY')
    tree.links.new(GI("Фальц_ширина_мм"), in_sock(n3bwhalf0, "Value"))
    in_sock(n3bwhalf0, "Value_001").default_value = 0.5

    n3bwhalf1 = add_node(tree, "ShaderNodeMath", "3b.whalf1",
                          "+ taper_offset_мм", 2640, -600, operation='ADD')
    link(tree, n3bwhalf0, "Value", n3bwhalf1, "Value")
    link(tree, n3btaperoff, "Value", n3bwhalf1, "Value_001")

    n3bwhalf = add_node(tree, "ShaderNodeMath", "3b.whalf",
                         "half_width_мм = max(..., 0.1) - не даём схлопнуться",
                         2900, -600, operation='MAXIMUM')
    link(tree, n3bwhalf1, "Value", n3bwhalf, "Value")
    in_sock(n3bwhalf, "Value_001").default_value = 0.1

    n3bwhalfcols = add_node(tree, "ShaderNodeMath", "3b.whalfcols",
                             "half_width в колонках = half_width_мм / (мм/колонка)",
                             3160, -600, operation='DIVIDE')
    link(tree, n3bwhalf, "Value", n3bwhalfcols, "Value")
    link(tree, n3bmmclamp, "Value", n3bwhalfcols, "Value_001")

    n3bedgecols0 = add_node(tree, "ShaderNodeMath", "3b.edgecols0",
                             "радиус кромки в колонках = Кромка_радиус_мм / (мм/колонка)",
                             3160, -750, operation='DIVIDE')
    tree.links.new(GI("Кромка_радиус_мм"), in_sock(n3bedgecols0, "Value"))
    link(tree, n3bmmclamp, "Value", n3bedgecols0, "Value_001")

    n3bedgecols = add_node(tree, "ShaderNodeMath", "3b.edgecols",
                            "= max(..., 0.02) - защита от деления на 0 ниже",
                            3420, -750, operation='MAXIMUM')
    link(tree, n3bedgecols0, "Value", n3bedgecols, "Value")
    in_sock(n3bedgecols, "Value_001").default_value = 0.02

    frameWidth = make_frame(
        tree, "3b-width. ШИРИНА ДНА ФАЛЬЦА (мм->колонки, с калибровкой и скосом)",
        [n3bwhalf0, n3bwhalf1, n3bwhalf, n3bwhalfcols, n3bedgecols0, n3bedgecols])

    # -- плоский "колокол" с плоским дном (plateau): 1 внутри половины
    # ширины дна, гладкий косинусный спуск к 0 на ширине радиуса кромки,
    # 0 дальше. Это и есть приближение прямоугольного (с чуть скруглёнными
    # краями) сечения фальца, в отличие от прежнего гладкого cos-купола
    # на всю полосу.
    n3bd2 = add_node(tree, "ShaderNodeMath", "3b.d2", "d2 = distSeam - half_width_колонок",
                      3420, -400, operation='SUBTRACT')
    link(tree, n3bdist, "Value", n3bd2, "Value")
    link(tree, n3bwhalfcols, "Value", n3bd2, "Value_001")

    n3bd2clamp = add_node(tree, "ShaderNodeMath", "3b.d2clamp", "d2 = max(d2, 0)",
                           3680, -400, operation='MAXIMUM')
    link(tree, n3bd2, "Value", n3bd2clamp, "Value")
    in_sock(n3bd2clamp, "Value_001").default_value = 0.0

    n3btt = add_node(tree, "ShaderNodeMath", "3b.tt", "tt = d2 / радиус_кромки_колонок",
                      3940, -400, operation='DIVIDE')
    link(tree, n3bd2clamp, "Value", n3btt, "Value")
    link(tree, n3bedgecols, "Value", n3btt, "Value_001")

    n3bttclamp = add_node(tree, "ShaderNodeMath", "3b.ttclamp", "tt = min(tt, 1)",
                           4200, -400, operation='MINIMUM')
    link(tree, n3btt, "Value", n3bttclamp, "Value")
    in_sock(n3bttclamp, "Value_001").default_value = 1.0

    n3bpi = add_node(tree, "ShaderNodeMath", "3b.pi", "* пи", 4460, -400, operation='MULTIPLY')
    link(tree, n3bttclamp, "Value", n3bpi, "Value")
    in_sock(n3bpi, "Value_001").default_value = 3.14159265358979

    n3bcos = add_node(tree, "ShaderNodeMath", "3b.cos", "cos(...)", 4720, -400, operation='COSINE')
    link(tree, n3bpi, "Value", n3bcos, "Value")

    n3badd1 = add_node(tree, "ShaderNodeMath", "3b.add1", "+1", 4980, -400, operation='ADD')
    link(tree, n3bcos, "Value", n3badd1, "Value")
    in_sock(n3badd1, "Value_001").default_value = 1.0

    n3bplateau = add_node(tree, "ShaderNodeMath", "3b.plateau",
                           "plateau(col) = *0.5 (плоские 1 на дне фальца, гладко к 0 за кромкой)",
                           5240, -400, operation='MULTIPLY')
    link(tree, n3badd1, "Value", n3bplateau, "Value")
    in_sock(n3bplateau, "Value_001").default_value = 0.5

    frame3b_calc = make_frame(
        tree, "3b-calc. distSeam (в колонках) И ПЛОСКОЕ ДНО 'plateau' (col=0 и col=M/2)",
        [n3bM, n3bidx, n3bcol, n3bd0a, n3bd0, n3bd1r, n3bd1, n3bdist,
         n3bd2, n3bd2clamp, n3btt, n3bttclamp, n3bpi, n3bcos, n3badd1, n3bplateau])

    # -- защита от самопересечения: смещения не должны превышать разумной
    # доли толщины стенки --
    n3bwallcap = add_node(tree, "ShaderNodeMath", "3b.wallcap",
                           "предел_язычок = Толщина_стенки_мм * 0.8", 3940, -900, operation='MULTIPLY')
    tree.links.new(GI("Толщина_стенки_мм"), in_sock(n3bwallcap, "Value"))
    in_sock(n3bwallcap, "Value_001").default_value = 0.8

    n3bwallcap2 = add_node(tree, "ShaderNodeMath", "3b.wallcap2",
                            "предел_утопление = Толщина_стенки_мм * 0.5", 3940, -1000,
                            operation='MULTIPLY')
    tree.links.new(GI("Толщина_стенки_мм"), in_sock(n3bwallcap2, "Value"))
    in_sock(n3bwallcap2, "Value_001").default_value = 0.5

    n3boutdepth = add_node(tree, "ShaderNodeMath", "3b.outdepth",
                            "утопление_мм_безоп = min(Фальц_утопление_мм, предел_утопление)",
                            4200, -1000, operation='MINIMUM')
    tree.links.new(GI("Фальц_утопление_мм"), in_sock(n3boutdepth, "Value"))
    link(tree, n3bwallcap2, "Value", n3boutdepth, "Value_001")

    n3boffouter = add_node(tree, "ShaderNodeMath", "3b.offouter",
                            "offset_утопление = plateau * утопление_мм_безоп",
                            5240, -1000, operation='MULTIPLY')
    link(tree, n3bplateau, "Value", n3boffouter, "Value")
    link(tree, n3boutdepth, "Value", n3boffouter, "Value_001")

    n3bMdepth = add_node(tree, "ShaderNodeMath", "3b.Mdepth",
                          "язычок_мм_безоп = min(Фальц_глубина_мм, предел_язычок)",
                          4200, -850, operation='MINIMUM')
    tree.links.new(GI("Фальц_глубина_мм"), in_sock(n3bMdepth, "Value"))
    link(tree, n3bwallcap, "Value", n3bMdepth, "Value_001")

    n3boffM = add_node(tree, "ShaderNodeMath", "3b.offM",
                        "offset_язычок(male) = plateau * язычок_мм_безоп",
                        5240, -850, operation='MULTIPLY')
    link(tree, n3bplateau, "Value", n3boffM, "Value")
    link(tree, n3bMdepth, "Value", n3boffM, "Value_001")

    n3bFdepth0 = add_node(tree, "ShaderNodeMath", "3b.Fdepth0",
                           "канал_мм = язычок_мм_безоп + Фальц_зазор_мм",
                           4460, -700, operation='ADD')
    link(tree, n3bMdepth, "Value", n3bFdepth0, "Value")
    tree.links.new(GI("Фальц_зазор_мм"), in_sock(n3bFdepth0, "Value_001"))

    n3bwallcap3 = add_node(tree, "ShaderNodeMath", "3b.wallcap3",
                            "предел_канал = Толщина_стенки_мм * 1.5", 4460, -600,
                            operation='MULTIPLY')
    tree.links.new(GI("Толщина_стенки_мм"), in_sock(n3bwallcap3, "Value"))
    in_sock(n3bwallcap3, "Value_001").default_value = 1.5

    n3bFdepth = add_node(tree, "ShaderNodeMath", "3b.Fdepth",
                          "канал_мм_безоп = min(канал_мм, Толщина_стенки_мм*1.5)",
                          4720, -700, operation='MINIMUM')
    link(tree, n3bFdepth0, "Value", n3bFdepth, "Value")
    link(tree, n3bwallcap3, "Value", n3bFdepth, "Value_001")

    n3boffF = add_node(tree, "ShaderNodeMath", "3b.offF",
                        "offset_канал(female) = plateau * канал_мм_безоп",
                        5240, -700, operation='MULTIPLY')
    link(tree, n3bplateau, "Value", n3boffF, "Value")
    link(tree, n3bFdepth, "Value", n3boffF, "Value_001")

    frameDepths = make_frame(
        tree, "3b-depth. ГЛУБИНЫ СО ЗАЩИТОЙ ОТ САМОПЕРЕСЕЧЕНИЯ "
        "(утопление обеих половин / язычок male / канал female)",
        [n3bwallcap, n3bwallcap2, n3bwallcap3, n3boutdepth, n3boffouter,
         n3bMdepth, n3boffM, n3bFdepth0, n3bFdepth, n3boffF])

    # -- НАРУЖНАЯ поверхность: ОБЕИХ половин утапливаем ОДИНАКОВО у шва
    # (симметрично) - значит у передней и задней половины снаружи не
    # возникает взаимного уступа, только тонкая (<= Фальц_утопление_мм)
    # едва заметная линия стыка.
    n3bcommondir = named_attr(tree, "3b.commondir", "Читаем 'dir' (утопление, общее)",
                               5240, -1150, "dir", 'FLOAT_VECTOR')
    n3bcommonvec = add_node(tree, "ShaderNodeVectorMath", "3b.commonvec",
                             "смещение = dir * offset_утопление", 5500, -1100, operation='SCALE')
    link(tree, n3bcommondir, "Attribute_Vector", n3bcommonvec, "Vector")
    link(tree, n3boffouter, "Value", n3bcommonvec, "Scale")

    n3bcommonpos = add_node(tree, "GeometryNodeInputPosition", "3b.commonpos",
                             "Position (внешняя, расширенная)", 5240, -1250)
    n3bcommonnewpos = add_node(tree, "ShaderNodeVectorMath", "3b.commonnewpos",
                                "новая позиция = позиция - смещение", 5760, -1150,
                                operation='SUBTRACT')
    link(tree, n3bcommonpos, "Position", n3bcommonnewpos, "Vector")
    link(tree, n3bcommonvec, "Vector", n3bcommonnewpos, "Vector_001")

    n_outer_common = add_node(tree, "GeometryNodeSetPosition", "3b.outCommon",
                               "Внешние_точки (обе половины) - утоплены у швов, БЕЗ уступа",
                               6020, -1150)
    link(tree, n07b, "Geometry", n_outer_common, "Geometry")
    link(tree, n3bcommonnewpos, "Vector", n_outer_common, "Position")

    frameOuter = make_frame(
        tree, "3b-outer. СИММЕТРИЧНОЕ УТОПЛЕНИЕ НАРУЖНОЙ ПОВЕРХНОСТИ (обе половины одинаково)",
        [n3bcommondir, n3bcommonvec, n3bcommonpos, n3bcommonnewpos, n_outer_common])

    # -- ВНУТРЕННЯЯ поверхность: Перед (male) получает язычок (толще
    # изнутри, к наружной поверхности), Зад (female) получает ответный
    # канал (тоньше изнутри) - интерлок скрыт внутри стенки, наружу
    # не выходит.
    n3bmdir = named_attr(tree, "3b.mdir", "Читаем 'dir' (язычок, male)",
                          5240, -1400, "dir", 'FLOAT_VECTOR')
    n3bmvec = add_node(tree, "ShaderNodeVectorMath", "3b.mvec",
                        "смещение = dir * offset_язычок", 5500, -1350, operation='SCALE')
    link(tree, n3bmdir, "Attribute_Vector", n3bmvec, "Vector")
    link(tree, n3boffM, "Value", n3bmvec, "Scale")

    n3binnerpos = add_node(tree, "GeometryNodeInputPosition", "3b.innerpos",
                            "Position (внутренняя, номинальная)", 5240, -1550)

    n3bmnewpos = add_node(tree, "ShaderNodeVectorMath", "3b.mnewpos",
                           "male inner = позиция + смещение (толще, к наружи)",
                           5760, -1350, operation='ADD')
    link(tree, n3binnerpos, "Position", n3bmnewpos, "Vector")
    link(tree, n3bmvec, "Vector", n3bmnewpos, "Vector_001")

    n_inner_male = add_node(tree, "GeometryNodeSetPosition", "3b.inM",
                             "Внутренние_точки (male, перед) - язычок у швов", 6020, -1350)
    link(tree, n13, "Geometry", n_inner_male, "Geometry")
    link(tree, n3bmnewpos, "Vector", n_inner_male, "Position")

    n3bfdir = named_attr(tree, "3b.fdir", "Читаем 'dir' (канал, female)",
                          5240, -1700, "dir", 'FLOAT_VECTOR')
    n3bfvec = add_node(tree, "ShaderNodeVectorMath", "3b.fvec",
                        "смещение = dir * offset_канал", 5500, -1650, operation='SCALE')
    link(tree, n3bfdir, "Attribute_Vector", n3bfvec, "Vector")
    link(tree, n3boffF, "Value", n3bfvec, "Scale")

    n3bfnewpos = add_node(tree, "ShaderNodeVectorMath", "3b.fnewpos",
                           "female inner = позиция - смещение (тоньше, канал)",
                           5760, -1650, operation='SUBTRACT')
    link(tree, n3binnerpos, "Position", n3bfnewpos, "Vector")
    link(tree, n3bfvec, "Vector", n3bfnewpos, "Vector_001")

    n_inner_female = add_node(tree, "GeometryNodeSetPosition", "3b.inF",
                               "Внутренние_точки (female, зад) - канал у швов", 6020, -1650)
    link(tree, n13, "Geometry", n_inner_female, "Geometry")
    link(tree, n3bfnewpos, "Vector", n_inner_female, "Position")

    frame3b = make_frame(
        tree, "3b. ФАЛЬЦ 'ЧЕТВЕРТЬ': язычок (male, перед) и ответный канал (female, зад) "
        "на внутренней поверхности - скрыто внутри стенки",
        [n3bmdir, n3bmvec, n3binnerpos, n3bmnewpos, n_inner_male,
         n3bfdir, n3bfvec, n3bfnewpos, n_inner_female])
    ...
    tree.links.new(out_sock(n_outer_common, "Geometry"), n16.inputs["Внешние_точки"])
    ...
    tree.links.new(out_sock(n_outer_common, "Geometry"), n17.inputs["Внешние_точки"])
    tree.links.new(out_sock(n_inner_female, "Geometry"), n17.inputs["Внутренние_точки"])
