# -*- coding: utf-8 -*-
"""
Автоматическая проверка Группы 4 ("NK.4_Закладные"), нанизанной на
полную цепочку Группа1 -> Группа2 -> Группа3 -> Группа4.

Запуск:
    blender --background --python scripts/verify_group4.py -- <out_dir>
"""
import math
import os
import sys
...
from build_group3_shell import SAMPLER_NAME
...
def build_full_master():
    ...
    # ВАЖНО: переиспользуем sampler_tree, который Группа 3 уже создала
    # (build_shell_all -> build_sampler() внутри) - иначе Группа 4
    # пересоздаст группу с тем же именем и оборвёт ссылки узлов Group
    # внутри уже собранной Группы 3 (см. комментарий в build_all_groups).
    ...
    master = bpy.data.node_groups.new("NK.Master_full", "GeometryNodeTree")
    master.interface.new_socket(name="Geometry", in_out='OUTPUT', socket_type='NodeSocketGeometry')
    mod.node_group = master

    gout = master.nodes.new("NodeGroupOutput"); gout.location = (1200, 0)
    g1 = master.nodes.new("GeometryNodeGroup"); g1.node_tree = axis_tree; g1.location = (-600, 0)
    g2 = master.nodes.new("GeometryNodeGroup"); g2.node_tree = profile_tree; g2.location = (-200, 0)
    g3 = master.nodes.new("GeometryNodeGroup"); g3.node_tree = shell_tree; g3.location = (200, 0)
    g4 = master.nodes.new("GeometryNodeGroup"); g4.node_tree = inserts_tree; g4.location = (600, 0)
...
def run_case(label, g4_params, out_dir):
    ...
    for outname in ["Перед", "Зад"]:
        master.links.new(g4.outputs[outname], gout_master.inputs["Geometry"])
        deps = bpy.context.evaluated_depsgraph_get()
        ...
        bm.free()
        oe.to_mesh_clear()
    master.links.new(g4.outputs["VIS_Вместе"], gout_master.inputs["Geometry"])
    ...
    scene.render.filepath = os.path.join(out_dir, f"group4_{label}.png")
...
def main():
    argv = sys.argv
    out_dir = argv[argv.index("--") + 1] if "--" in argv else "/tmp/nk_preview4"
