...
def build():
    ...
    # Небольшой допуск (0.5мм) на строгое сравнение "высота BBox
    # кривой <= заданной длины": ресемплинг кривой по длине дуги -
    # численное приближение, а не точное совпадение до бита. На
    # Blender 5.1 обнаружено (при переходе на эту версию), что
    # результат может на ~0.02мм превышать заданную длину чисто из-за
    # иначе округляемой арифметики решателя - пренебрежимо для
    # 3D-печати, но ломало точное "<=" без допуска.
    v16tol = add_node(tree, "ShaderNodeMath", "V1.6tol",
                       "заданная длина + 0.5мм допуска (числ. погрешность ресемплинга)",
                       2850, 780, operation='ADD')
    tree.links.new(GI("Длина_сегмента_мм"), in_sock(v16tol, "Value"))
    in_sock(v16tol, "Value_001").default_value = 0.5

    v16 = add_node(tree, "FunctionNodeCompare", "V1.6",
                    "Высота BBox <= заданной длины (+0.5мм допуск)?", 3150, 700,
                    data_type='FLOAT', operation='LESS_EQUAL')
    link(tree, v15, "Z", v16, "A")
    link(tree, v16tol, "Value", v16, "B")
