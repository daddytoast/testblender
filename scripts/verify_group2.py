...
# Параметры, которые в мастер-дереве идут из Группы 1 (Длина_сегмента_мм,
# Сторона_ноги) - у g2 эти сокеты теперь заняты линком из g1, поэтому в
# тестах такие ключи адресуются на g1, а не на g2.
G1_PARAMS = {"Длина_сегмента_мм", "Сторона_ноги"}
...
def main():
    ...
    # РЕГРЕССИЯ: баг "обхват в нижней точке срабатывает сверху" -
    # Точка1/Точка2 намеренно переданы В ОБРАТНОМ порядке (Точка1 теперь
    # выше по мм-от-колена, чем Точка2). Раньше From Min/Max в Map Range
    # не сортировались -> интерполяция инвертировалась. Теперь сортировка
    # по T1<T2 (узлы 2.13cmp + 4 Switch) должна дать ТОТ ЖЕ результат, что
    # и "default" (там просто Точка1/Точка2 физически не переставлены
    # местами, но семантически совпадают с этим случаем при swap R1<->R2).
    results.append(run_case("swapped_points_regression", {
        "Обхват_Точка1_мм": 340.0,      # было Точка2 в default
        "Точка1_мм_от_колена": 105.0,   # было Точка2 в default
        "Обхват_Точка2_мм": 220.0,      # было Точка1 в default
        "Точка2_мм_от_колена": 308.0,   # было Точка1 в default
    }, out_dir))

    # Асимметрия гребня: смещение вбок + проверка зеркаливания через
    # Сторона_ноги (значение читается из Группы 1, см. G1_PARAMS).
    results.append(run_case("crest_shift_right", {
        "Гребень_смещение_град": 20.0,
        "Сторона_ноги": 1.0,
    }, out_dir))
    results.append(run_case("crest_shift_left", {
        "Гребень_смещение_град": 20.0,
        "Сторона_ноги": -1.0,
    }, out_dir))

    # Овальность у лодыжки (новая секция 7b).
    results.append(run_case("ovality_max", {
        "Овальность_у_лодыжки_мм": 15.0,
        "Овальность_протяжённость_доля": 0.5,
    }, out_dir))

    # мм-параметры точек/икры теперь абсолютные от колена - проверяем
    # зажатие при значении больше самой длины сегмента (должно зажаться
    # к 0..1, без ошибок/вырождения).
    results.append(run_case("mm_params_clamp", {
        "Точка1_мм_от_колена": 900.0,
        "Точка2_мм_от_колена": 900.0,
        "Высота_икры_мм_от_колена": 900.0,
    }, out_dir))
